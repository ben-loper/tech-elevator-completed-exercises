...
SELECT name, continent, gnp, lifeexpectancy FROM country WHERE lifeexpectancy >= 70 AND lifeexpectancy IS NOT NULL AND gnp BETWEEN 1 AND 100;

-- 21. The per capita GNP (i.e. GNP divided by population) in US Dollars of all countries in Europe (46 rows)
SELECT name, FORMAT((gnp / population), 'c', 'en-us') as 'Per capita GNP' from country WHERE continent = 'Europe';

-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
Select name, YEAR(CURRENT_TIMESTAMP) - indepyear as 'Number of years since independence' FROM country WHERE indepyear IS NOT NULL;